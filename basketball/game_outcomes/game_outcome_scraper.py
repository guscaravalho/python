...
def scrape_game_outcomes(team):
    ...
    if response.status_code == 200:
        ...
        if table:
            ...
            folder_path = 'game_outcome_csv_files'
...
def generate_team_list(list_of_nba_teams):
    with open(list_of_nba_teams, 'r') as file:
        reader = csv.reader(file)
        next(reader)  # Skip the header row
        
        for row in reader:
            team = row[0]  # team code is first column
            scrape_game_outcomes(team)  # This command invokes the function above and inserts the team name

list_of_nba_teams = 'nba_teams.csv'

team = generate_team_list(list_of_nba_teams)